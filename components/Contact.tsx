...
function TransmissionLog() {
  ...
  return (
    <div style={{background:"#030303",border:"1px solid rgba(26,40,72,0.4)",borderLeft:"2px solid #1A2848",padding:"14px 18px",fontFamily:"var(--font-mono)",fontSize:11,lineHeight:2}}>
      <div style={{color:"rgba(255,255,255,0.35)",letterSpacing:"0.18em",marginBottom:8,paddingBottom:8,borderBottom:"1px solid rgba(26,40,72,0.3)",fontSize:11}}>SYSTEM_LOG // INTAKE_v2026</div>
      {ENTRIES.map((e,i)=>(
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.2em",color:focused?"#F25C43":"rgba(255,255,255,0.7)",textTransform:"uppercase",transition:"color 0.3s",fontWeight:400}}>{label}</span>
        {error && <span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"#F25C43",letterSpacing:"0.1em"}}>⚠ {error}</span>}
      </div>
...
function UrlScanner({ onUrlScanned }: { onUrlScanned?: (url: string | null) => void }) {
  ...
  return (
    <div style={{width:"100%",display:"flex",flexDirection:"column",gap:16}}>
      <div>
        <div style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.3em",color:"#F25C43",marginBottom:10,display:"flex",alignItems:"center",gap:8}}>
        ...
        <p style={{fontFamily:"var(--font-sans)",fontSize:16,fontWeight:300,color:"rgba(255,255,255,0.65)",lineHeight:1.8,borderLeft:"2px solid rgba(26,40,72,0.6)",paddingLeft:18,marginBottom:0}}>
      ...
      <div>
        ...
        {failed && <div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"#F25C43",letterSpacing:"0.14em",marginTop:6}}>⚠ INVALID URL — try including the full address</div>}
        {!scanned&&!scanning&&!failed && <div style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.16em",color:"rgba(255,255,255,0.3)",marginTop:6}}>PRESS ENTER OR SCAN ↑  ·  SCREENSHOT LOADS IN ~5s</div>}
      </div>
              ...
              <>
                ...
                {imgLoaded && (
                  <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 35%,rgba(3,3,3,0.88) 100%)",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"16px 18px"}}>
                    ...
                    <div style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.16em",color:"rgba(255,255,255,0.5)"}}>SITE SCANNED · SEND US THIS URL IN THE FORM →</div>
                  </div>
                )}
              </>
            )}
        ...
        >
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18,opacity:entered?1:0,animation:entered?"slideL 0.8s ease forwards":"none"}}>
            ...
            <div style={{border:"1px solid rgba(26,40,72,0.3)",padding:"16px 16px 10px",background:"rgba(26,40,72,0.04)"}}>
              <div style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.2em",color:"rgba(255,255,255,0.35)",marginBottom:8}}>LIVE_SIGNAL_MAP</div>
              <SignalMap/>
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:2}}>
              {[
                ...
                >
                  <span style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.22em",color:"#F25C43"}}>{item.label}</span>
                  <span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(255,255,255,0.6)",letterSpacing:"0.1em"}}>{item.value}</span>
                </a>
              ))}
            </div>

            <div             style={{display:"inline-flex",alignItems:"center",gap:10,padding:"9px 15px",border:"1px solid rgba(26,40,72,0.5)",background:"rgba(26,40,72,0.1)",fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.16em",color:"rgba(255,255,255,0.55)"}}>
              ...
              PROVISIONING <span style={{color:"#fff",marginLeft:4}}>MAY 2026</span> SPRINTS
            </div>
          </div>
          ...
          <div style={{opacity:entered?1:0,animation:entered?"slideR 0.8s ease 0.12s forwards":"none"}}>
            <div style={{background:"#0A0A0A",border:"1px solid rgba(255,255,255,0.08)",borderTop:"1px solid rgba(26,40,72,0.5)",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{padding:"13px 20px",borderBottom:"1px solid rgba(255,255,255,0.07)",background:"rgba(26,40,72,0.1)",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(255,255,255,0.5)",letterSpacing:"0.2em"}}>INTAKE_TERMINAL.form</span>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <div style={{width:5,height:5,borderRadius:"50%",background:"#F25C43",animation:"blink 2s ease infinite"}}/>
                  <span style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(255,255,255,0.4)",letterSpacing:"0.14em"}}>SECURE_CHANNEL</span>
                </div>
              </div>
              ...
              <div style={{padding:"32px 32px 28px"}}>
                <div style={{marginBottom:32}}>
                  ...
                  <div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(255,255,255,0.5)",letterSpacing:"0.18em"}}>
                  ...
                  {scannedUrl && (
                    <div style={{marginTop:12,display:"inline-flex",alignItems:"center",gap:8,padding:"5px 12px",border:"1px solid rgba(242,92,67,0.25)",background:"rgba(242,92,67,0.06)",animation:"fadeInUp 0.4s ease forwards"}}>
                      ...
                      <span style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.14em",color:"rgba(255,255,255,0.6)"}}>SITE SCANNED:</span>
                       <span style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.1em",color:"#F25C43",maxWidth:200,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{scannedUrl.replace("https://","")}</span>
                    </div>
                  )}
                </div>
                ...
                {status === "success" ? (
                  <div style={{padding:"40px 20px",border:"1px solid rgba(26,40,72,0.5)",borderTop:"2px solid #F25C43",background:"rgba(26,40,72,0.08)",display:"flex",flexDirection:"column",alignItems:"center",gap:16,textAlign:"center"}}>
                    ...
                    <div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"rgba(255,255,255,0.5)",letterSpacing:"0.18em"}}>RESPONSE_SLA: 24H // PIPELINE_INITIALIZED</div>
...
/>
                    ...
                    <div>
                      <div style={{fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.22em",color:"rgba(255,255,255,0.7)",marginBottom:12,textTransform:"uppercase"}}>
                        Estimated_Budget
                      </div>
                        ...
                        {BUDGET_OPTIONS.map(opt=>{
                          ...
                          return (
                            ...
                            >
                              ...
                              <span style={{fontFamily:"var(--font-mono)",fontSize:10,letterSpacing:"0.12em",color:sel?"rgba(242,92,67,0.7)":"rgba(255,255,255,0.3)"}}>{opt.sub}</span>
                    ...
                    {status==="error" && (
                      <div style={{fontFamily:"var(--font-mono)",fontSize:11,color:"#F25C43",letterSpacing:"0.18em",textAlign:"center"}}>
