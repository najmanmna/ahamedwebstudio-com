...
// ─── TYPES ────────────────────────────────────────────────────────────────────
type NodePoint = { x: number; y: number; label: string; active: boolean };
type Pulse = { id: number; from: NodePoint; to: NodePoint; progress: number };
    ...
    <div style={{ position: "relative", width: "100%", height: 140 }}>
      <svg width="100%" height="100%" viewBox="0 0 100 60" preserveAspectRatio="xMidYMid meet">
        ...
        {pulses.map(p => {
          ...
          return (
            <g key={p.id}>
              <line x1={p.from.x} y1={p.from.y} x2={p.to.x} y2={p.to.y} stroke="rgba(242,92,67,0.1)" strokeWidth="0.4" strokeDasharray="2 2"/>
              <circle cx={x} cy={y} r="1.2" fill="#F25C43" style={{ filter: "drop-shadow(0 0 2px #F25C43)" }}/>
            </g>
          );
        })}
        {nodes.map(n => (
      ...
      <div style={{ position:"absolute",bottom:0,left:0,fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"0.18em",color:"rgba(255,255,255,0.5)" }}>
...
function TransmissionLog() {
  ...
  return (
    <div style={{ background:"#030303",border:"1px solid rgba(26,40,72,0.4)",borderLeft:"2px solid #1A2848",padding:"14px 18px",fontFamily:"var(--font-mono)",fontSize:9,lineHeight:2 }}>
      <div style={{ color:"rgba(255,255,255,0.45)",letterSpacing:"0.18em",marginBottom:8,paddingBottom:8,borderBottom:"1px solid rgba(26,40,72,0.3)",fontSize:8 }}>SYSTEM_LOG // INTAKE_v2026</div>
      {ENTRIES.map((e, i) => (
        <div key={i} style={{ color:e.ok?"#F25C43":"rgba(255,255,255,0.6)",opacity:visible.includes(i)?1:0,transform:visible.includes(i)?"translateX(0)":"translateX(-8px)",transition:"opacity 0.3s ease, transform 0.3s ease",letterSpacing:"0.07em" }}>
          <span style={{ color:e.ok?"rgba(100,130,200,0.9)":"rgba(255,255,255,0.3)",marginRight:8 }}>›</span>{e.text}
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <span style={{ fontFamily:"var(--font-mono)",fontSize:10,letterSpacing:"0.22em",color:focused?"#F25C43":"rgba(255,255,255,0.65)",textTransform:"uppercase",transition:"color 0.3s" }}>{label}</span>
        {error && <span style={{ fontFamily:"var(--font-mono)",fontSize:9,color:"#F25C43",letterSpacing:"0.1em" }}>⚠ {error}</span>}
      </div>
      <div style={{ position:"relative" }}>
        <div style={{ position:"absolute",top:0,bottom:0,left:0,width:2,background:focused?"#F25C43":"rgba(26,40,72,0.5)",transition:"background 0.3s",boxShadow:focused?"0 0 8px rgba(242,92,67,0.4)":"none" }} />
        <El
          {...props}
          ...
          style={{ width:"100%",background:focused?"rgba(242,92,67,0.025)":"rgba(26,40,72,0.04)",border:`1px solid ${error?"rgba(242,92,67,0.4)":focused?"rgba(242,92,67,0.3)":"rgba(26,40,72,0.35)"}`,borderLeft:"none",color:"#fff",fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:"0.05em",padding:isTA?"14px 16px":"0 16px",height:isTA?"auto":50,resize:"none",outline:"none",transition:"all 0.3s",display:"block" }}
        />
      </div>
    </div>
  );
...
// ─── URL SCANNER — the magic ──────────────────────────────────────────────────
// Paste a URL → live screenshot appears in browser mockup → "We'll rebuild this."
function UrlScanner({ onUrlScanned }: { onUrlScanned: (url: string | null) => void }) {
  ...
  const handleKey = (e: React.KeyboardEvent) => { if (e.key === "Enter") scan(); };
  ...
  return (
    <div style={{ width:"100%", display:"flex", flexDirection:"column", gap: 16 }}>
      ...
      <div>
        ...
        <h2 style={{ fontFamily:"var(--font-display)",fontSize:"clamp(2.4rem,4.5vw,4.2rem)",color:"#fff",lineHeight:0.95,textTransform:"uppercase",marginBottom:20 }}>
          READY TO<br/>INITIATE<br/>
          <span style={{ WebkitTextStroke:"1px rgba(255,255,255,0.18)",color:"transparent" }}>ENGINEERING?</span>
        </h2>
        <p style={{ fontFamily:"var(--font-sans)",fontSize:14,fontWeight:300,color:"rgba(255,255,255,0.55)",lineHeight:1.8,borderLeft:"2px solid rgba(26,40,72,0.6)",paddingLeft:18,marginBottom:0 }}>
      ...
      <div style={{ position:"relative" }}>
        <div style={{ display:"flex", gap:0 }}>
          {/* Browser favicon strip */}
          <div style={{
            height:52, width:44, flexShrink:0,
            ...
            {scanning ? (
              ...
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            )}
          </div>
          ...
          <input
            ...
            style={{
              ...
              border:`1px solid ${failed?"rgba(242,92,67,0.5)":scanned?"rgba(242,92,67,0.3)":"rgba(26,40,72,0.4)"}`,
          ...
          <button
            ...
            style={{
              ...
              background: scanned ? "rgba(26,40,72,0.3)" : "#F25C43",
              border:`1px solid ${scanned ? "rgba(26,40,72,0.5)" : "transparent"}`,
              color: scanned ? "rgba(255,255,255,0.6)" : "#000",
              fontFamily:"var(--font-mono)",fontSize:10,letterSpacing:"0.2em",
              cursor: scanning || (!url && !scanned) ? "default" : "pointer",
              transition:"all 0.3s",
              opacity: (!url && !scanned) ? 0.4 : 1,
              whiteSpace:"nowrap",
        ...
        {failed && (
          <div style={{ fontFamily:"var(--font-mono)",fontSize:9,color:"#F25C43",letterSpacing:"0.14em",marginTop:6 }}>
            ⚠ INVALID URL — try including the full address
          </div>
        )}
        {!scanned && !scanning && !failed && (
          <div style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.16em",color:"rgba(255,255,255,0.4)",marginTop:6 }}>
            PRESS ENTER OR SCAN ↑  ·  SCREENSHOT LOADS IN ~5s
          </div>
        )}
      </div>

      {/* Browser mockup — appears after scan */}
      {(scanning || scanned) && (
        <div style={{
          border:"1px solid rgba(26,40,72,0.4)",
          ...
          <div style={{ height:32,background:"#111",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",padding:"0 12px",gap:8 }}>
            ...
            <div style={{ flex:1,maxWidth:280,height:18,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:3,display:"flex",alignItems:"center",paddingInline:8,gap:5 }}>
              ...
              <span style={{ fontFamily:"var(--font-mono)",fontSize:8,color:"rgba(255,255,255,0.35)",letterSpacing:"0.04em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>
          ...
          <div style={{ position:"relative",height:200,overflow:"hidden",background:"#0A0A0A" }}>
            {scanning && (
              <div style={{ position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14 }}>
                ...
                <div style={{ fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"0.2em",color:"rgba(255,255,255,0.3)",animation:"blink 1.5s ease infinite" }}>SCANNING SITE...</div>
                <div style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.14em",color:"rgba(255,255,255,0.15)" }}>LOADING HEADLESS BROWSER</div>
              </div>
            )}
            {scanned && (
              <>
                {!imgLoaded && (
                  <div style={{ position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center" }}>
                    <div style={{ width:24,height:24,border:"1.5px solid rgba(26,40,72,0.5)",borderTopColor:"#F25C43",borderRadius:"50%",animation:"spin 0.8s linear infinite" }} />
                  </div>
                )}
                <img
                  src={scanned.src} alt="Site preview"
                ...
                {imgLoaded && (
                  <div style={{ position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 35%,rgba(3,3,3,0.88) 100%)",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"16px 18px" }}>
                    <div style={{ fontFamily:"var(--font-display)",fontSize:18,color:"#F25C43",letterSpacing:"0.04em",lineHeight:1,marginBottom:4 }}>
                      We'll rebuild this.
                    </div>
                    <div style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.16em",color:"rgba(255,255,255,0.4)" }}>
                      SITE SCANNED · SEND US THIS URL IN THE FORM →
                    </div>
                  </div>
                )}
              </>
...
export default function Contact() {
  ...
  const [form, setForm]         = useState<{name:string;email:string;company:string;message:string;budget:string}>({ name:"", email:"", company:"", message:"", budget:"" });
  ...
  // Pre-fill message with scanned URL
  useEffect(() => {
    if (scannedUrl) {
      setForm(prev => ({
        ...prev,
        message: prev.message ? prev.message : `Rebuild: ${scannedUrl}\n\n`,
      }));
    }
  }, [scannedUrl]);
  ...
  return (
    <>
      <style>{`
        ...
        input::placeholder,textarea::placeholder { color:rgba(255,255,255,0.15) !important; font-family:var(--font-mono); letter-spacing:0.05em; font-size:11px; }

        @media (max-width:960px)  { .contact-grid { grid-template-columns:1fr !important; } }
        @media (max-width:640px)  { .budget-row { flex-direction:column !important; } }
        @media (max-width:560px)  { .name-grid { grid-template-columns:1fr !important; } }
        @media (max-width:860px)  { #contact { padding-bottom: calc(80px + env(safe-area-inset-bottom,0px)) !important; } }
      `}</style>
      ...
      >
        ...
        >
          ...
          <div style={{
            ...
            opacity: entered?1:0,
            animation: entered?"slideL 0.8s ease forwards":"none",
          }}>
            {/* URL Scanner hero */}
            ...
            <div style={{ border:"1px solid rgba(26,40,72,0.3)",padding:"16px 16px 10px",background:"rgba(26,40,72,0.04)" }}>
              <div style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.2em",color:"rgba(255,255,255,0.5)",marginBottom:8 }}>LIVE_SIGNAL_MAP</div>
              <SignalMap />
            </div>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:2 }}>
              {[
                ...
                <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer"
                  style={{ display:"flex",flexDirection:"column",gap:4,padding:"12px 15px",border:"1px solid rgba(255,255,255,0.04)",background:"rgba(255,255,255,0.01)",textDecoration:"none",transition:"border-left-color 0.3s,background 0.3s",borderLeft:"2px solid transparent" }}
                  onMouseEnter={e => { e.currentTarget.style.borderLeftColor="#F25C43"; e.currentTarget.style.background="rgba(242,92,67,0.025)"; }}
                  onMouseLeave={e => { e.currentTarget.style.borderLeftColor="transparent"; e.currentTarget.style.background="rgba(255,255,255,0.01)"; }}
                >
                  <span style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.22em",color:"#F25C43" }}>{item.label}</span>
                  <span style={{ fontFamily:"var(--font-mono)",fontSize:10,color:"rgba(255,255,255,0.4)",letterSpacing:"0.1em" }}>{item.value}</span>
                </a>
              ))}
            </div>

            {/* Sprint badge */}
            <div style={{ display:"inline-flex",alignItems:"center",gap:10,padding:"9px 15px",border:"1px solid rgba(26,40,72,0.5)",background:"rgba(26,40,72,0.1)",fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"0.16em",color:"rgba(255,255,255,0.45)" }}>
          ...
          <div style={{
            opacity: entered?1:0,
            animation: entered?"slideR 0.8s ease 0.12s forwards":"none",
          }}>
            <div style={{
              background:"#0A0A0A",
              ...
              {/* Scan line on the form panel */}
              <div style={{ position:"absolute",left:0,right:0,height:1,background:"linear-gradient(to right,transparent,rgba(242,92,67,0.2),transparent)",pointerEvents:"none",zIndex:5,animation:"scanDown 6s linear infinite",top:0 }} />

              {/* Chrome */}
              <div style={{ padding:"13px 20px",borderBottom:"1px solid rgba(255,255,255,0.05)",background:"rgba(26,40,72,0.09)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                ...
                <span style={{ fontFamily:"var(--font-mono)",fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:"0.2em" }}>INTAKE_TERMINAL.form</span>
                <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                  <div style={{ width:5,height:5,borderRadius:"50%",background:"#F25C43",animation:"blink 2s ease infinite" }} />
                  <span style={{ fontFamily:"var(--font-mono)",fontSize:8,color:"rgba(255,255,255,0.55)",letterSpacing:"0.14em" }}>SECURE_CHANNEL</span>
                </div>
              </div>

              <div style={{ padding:"32px 32px 28px" }}>
                <div style={{ marginBottom:28 }}>
                  <h3 style={{ fontFamily:"var(--font-display)",fontSize:24,color:"#fff",letterSpacing:"0.04em",marginBottom:5,textTransform:"uppercase" }}>TRANSMIT DIRECTIVES</h3>
                  <div style={{ fontFamily:"var(--font-mono)",fontSize:8,color:"rgba(255,255,255,0.55)",letterSpacing:"0.18em" }}>
                  ...
                  {scannedUrl && (
                    <div style={{ marginTop:12,display:"inline-flex",alignItems:"center",gap:8,padding:"5px 12px",border:"1px solid rgba(242,92,67,0.25)",background:"rgba(242,92,67,0.06)",animation:"fadeInUp 0.4s ease forwards" }}>
                      ...
                      <span style={{ fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:"0.14em",color:"rgba(255,255,255,0.5)" }}>SITE SCANNED:</span>
                ...
                {status === "success" ? (
                  <div style={{ padding:"40px 20px",border:"1px solid rgba(26,40,72,0.5)",borderTop:"2px solid #F25C43",background:"rgba(26,40,72,0.08)",display:"flex",flexDirection:"column",alignItems:"center",gap:16,textAlign:"center" }}>
                    ...
                    <div style={{ fontFamily:"var(--font-mono)",fontSize:9,color:"rgba(255,255,255,0.4)",letterSpacing:"0.18em" }}>RESPONSE_SLA: 24H // PIPELINE_INITIALIZED</div>
                    <div style={{ width:"100%",height:1,background:"linear-gradient(to right,#1A2848,rgba(242,92,67,0.4),transparent)",marginTop:8 }} />
                    <div style={{ fontFamily:"var(--font-mono)",fontSize:10,color:"rgba(255,255,255,0.5)",letterSpacing:"0.14em" }}>hello@ahamedwebstudio.com</div>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} style={{ display:"flex",flexDirection:"column",gap:18 }}>
                    ...
                    {/* Budget */}
                    <div>
                      <div style={{ fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"0.22em",color:"rgba(255,255,255,0.55)",marginBottom:10,textTransform:"uppercase" }}>Estimated_Budget</div>
                      <div className="budget-row" style={{ display:"flex",gap:5,flexWrap:"wrap" }}>
                        {["< $500","$500–$1500","$1500–$5000","$5000+"].map(opt => (
                          <button key={opt} type="button" onClick={() => setForm(prev => ({ ...prev, budget: opt }))} style={{
                            padding:"5px 12px",
                            border:`1px solid ${form.budget===opt?"rgba(242,92,67,0.45)":"rgba(26,40,72,0.4)"}`,
                            background: form.budget===opt?"rgba(242,92,67,0.07)":"rgba(26,40,72,0.08)",
                            fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:"0.14em",
                            color: form.budget===opt?"#F25C43":"rgba(255,255,255,0.32)",
